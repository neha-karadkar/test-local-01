...
class Config:
    ...
    # Model configuration sets for LLM kwargs
    ...
    def _load_keyvault_secrets(cls):
        ...
        def _sanitize_kv_value(raw: str) -> str:
        ...
        for secret_name, refs in by_secret.items():
            try:
                ...
                if not secret or not secret.value:
                    ...
                    continue
                raw_value = _sanitize_kv_value(secret.value)
                has_json_key = any(json_key is not None for _, json_key in refs)
                if has_json_key:
                    data = _repair_json(raw_value)
...
def _initialize_config():
    ...
    for var_name in CONFIG_VARIABLES:
        ...
        if not value:
            # Exception: OBS_AZURE_SQL_TRUST_SERVER_CERTIFICATE defaults to "yes" if not found
